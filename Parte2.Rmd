...
Listo!
```{r}
head(Opcion1$hora_corregida,5)
```

Verificar
```{r}
range(Opcion1$hora_corregida)
```
...
```{r}
Opcion2$hora_corregida <- Opcion2$diahora - 3600*5
```
Nota que hora se resta a la columna diahora y las cinco horas se multiplican por los minutos y segundos.
...
```{r}
Opcion3$hora_corregida <- with_tz(Opcion3$Hora,tzone = "America/La_Paz")
```
